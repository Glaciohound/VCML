...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, args, command):
        ...
        self.split_mask, self.filenames, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships =\
            load_graphs(
        ...
        self.ind_to_classes, self.ind_to_attributes, self.ind_to_predicates = load_info(dict_file)
    ...
    def __getitem__(self, index):
        ...
        entry = {
            ...
            'gt_attributes': self.gt_attributes[index].copy(),
...
def assertion_checks(entry):
    ...
    if entry['gt_attributes'].shape[0] != num_gt:
        raise ValueError("GT attributes and GT boxes must have same number of examples")
...
def load_image_filenames(image_file, image_dir=VG_IMAGES):
    ...
    fns = glob(os.path.join(image_dir, '*'))
    '''
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
                filter_non_overlap=False, image_dir=VG_IMAGES):
    """
    ...
    raw_filenames = roi_h5['img_ids'][:].astype('U').tolist()
    filenames = [os.path.join(image_dir, filename+'.jpg') for filename in raw_filenames]
    ...
    print('--------> reading boxes')
    all_boxes = roi_h5['boxes_{}'.format(BOX_SCALE)][:].astype(np.float32)  # will index later
    ...
    for i in tqdm(range(len(image_index))):
        ...
        gt_attributes_i = all_attributes[im_to_first_box[i]:im_to_last_box[i]+1, :]
    ...
    return split_mask, filenames, boxes, gt_classes, gt_attributes, relationships
...
def load_info(info_file):
    ...
    ind_to_attributes = sorted(attribute_to_idx, key=lambda k: attribute_to_idx[k])
