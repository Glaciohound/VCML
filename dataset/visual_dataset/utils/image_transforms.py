...
class SquarePad(object):
    def __call__(self, img):
        ...
        img_padded = ImageOps.expand(
            img,
            border=(0, 0, max(h - w, 0), max(w - h, 0)),
...
def random_crop(
        img, boxes, box_scale,
        round_boxes=True, max_crop_fraction=0.1):
    """
    Randomly crops the image
    :param img: PIL image
    :param boxes: Ground truth boxes
    :param box_scale: This is the scale that the boxes are at (e.g. 1024 wide).
        We'll preserve that ratio
    :param round_boxes: Set this to true if to round the boxes to ints
...
class RandomOrder(object):
    ...
    def __call__(self, img):
        ...
        order = np.random.choice(
            len(self.transforms), size=num_to_pick)
        for i in order:
